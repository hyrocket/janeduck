      ...
      <div className="max-w-lg mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-4">
          <p className="text-xs text-gray-400">
            {userId ? `${decks.length} deck${decks.length !== 1 ? "s" : ""}` : ""}
          </p>
          {userId && (
            <Link
              href="/starred"
        ...
        {userId && decks.length === 0 && (
          <div className="text-center py-16 space-y-4">
            ...
            <Link href="/library" className="inline-block px-6 py-3 bg-yellow-400 hover:bg-yellow-500 text-yellow-900 font-bold text-sm rounded-2xl transition-colors">
              Browse Library
            </Link>
          </div>
        )}
        ...
        {decks.length > 0 && (
          <div className="space-y-3">
            {decks.map(deck => {
              ...
              return (
                <div key={deck.id as string} className="relative group">
                  <Link
                    href={`/deck/${deck.id}`}
