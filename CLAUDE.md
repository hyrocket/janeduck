...
## 개발 환경 — Windows PowerShell 주의사항

이 프로젝트는 Windows PowerShell 환경에서 작업한다.

- **git 명령은 Bash 툴**로 실행한다. PowerShell에서 git 실행 시 인코딩 문제로 자주 깨진다.
- **커밋 메시지는 `-m "한 줄"` 형식으로 짧게.** PowerShell here-string(`@'...'@`)은 깨지므로 멀티라인 커밋 메시지를 쓰지 않는다.
- PowerShell에서 `;`로 명령을 잇거나 한글이 포함된 명령은 깨질 수 있으니, 명령을 단순하게 유지한다.
- 경로에 괄호가 들어간 폴더명은 PowerShell에서 문제를 일으킬 수 있다.

---

## 협업 규칙

- **의견/검토 요청**("~가 낫지 않냐", "~이 맞냐")에는 답변만 한다. 명시적 지시("바꿔", "수정해") 확인 후 코드를 수정한다.

### 커밋 / 푸시 규칙

- **코드 수정 후 커밋/푸시는 사용자가 명시적으로 요청할 때만 한다.** 수정 완료 후 테스트할 시간을 준다.
- **auth, DB 스키마, API 핵심 로직처럼 프로덕션 영향이 큰 변경**은 커밋 전에 반드시 확인을 구한다.
- 푸시하면 Vercel 자동배포가 트리거된다. "커밋해줘"와 "푸시해줘"를 구분해서 받는다 — 명시적으로 푸시 요청이 없으면 커밋만 한다.
