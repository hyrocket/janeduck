...
- 큐의 재료는 3종 풀:
  - **Review**: 이미 학습한 단어. `review_priority`(§4-4) 높은 순으로 뽑음. (mastery 낮음 / 오래됨 / 자가평가 어렵다 / 덜 써봄)
  - **New**: 아직 한 번도 학습 안 한 단어 (`user_cards` 레코드 없음)
  - **Starred**: 별표 단어 (복습 시기 도래한 것 우선)
- **비율은 학습 진행도에 따라 변동** (가이드 — 정확한 수치는 `SRS_SPEC.md`에서 확정):
  - 진행 초반(학습률 낮음) → new 위주
  - 중반 → new/review 균형
  - 후반(학습률 높음) → review 위주
- **규칙 1 — "복습만 나오지 않는다":** new 단어가 남아 있는 한, 큐 전체가 review로만 채워지면 안 된다. new 최소 비율을 보장한다.
- **규칙 2 — 풀 부족 시 에러 없이 처리:** 어떤 풀이 목표 수량보다 적어도(예: 비율상 review 10장이 필요한데 review 풀에 3장뿐) 오류를 내지 않는다. 부족분은 다른 풀(new/weak/starred)에서 채우거나, 그만큼 적은 큐로 진행한다. **모든 단어가 한 풀에 몰린 경우(예: new가 동남, 전부 review)도 정상 동작해야 한다** — new가 없으면 review 100%가 정상이다.
- 별표만 모아 보는 별도 필터 학습 모드도 가능해야 한다 (`study_sessions.session_type`).
...
- `ai_score ≥ 8` → mastery **+1** (상한 5, 단 아래 scaffold 천장 적용)
- `ai_score 5~7` → mastery **유지**
- `ai_score ≤ 4` → mastery **-1** (하한 1 — untouched(0)으로는 내려가지 않음)

**갱신은 언제나 +1씩.** 점수가 아무리 높아도 한 번에 2 이상 오르지 않는다 — "한방 마스터" 금지. (hysteresis가 스텝에 내장됨)

**scaffold별 천장 — high scaffold 점수 인플레이션 방지:**
high scaffold는 sentence starter로 문장 뼈대를 거의 다 잡아주므로 점수가 7~9에 몰리는 경향이 있다. 그 점수로 mastery가 무한정 오르면 "도움받아 잘함"과 "혼자 잘함"이 구분되지 않는다. 따라서 `+1`은 **이번 시도의 scaffold에 따라 도달 천장**을 둔다:

| 이번 시도 scaffold | `ai_score ≥ 8`일 때 +1로 올라갈 수 있는 천장 |
|---|---|
| `high` | mastery **2까지** (2에서 high로는 더 못 올림) |
| `medium` | mastery **4까지** (4에서 medium으로는 더 못 올림) |
| `low` | mastery **5까지** |

- mastery 3 이상은 medium 또는 low scaffold 시도에서만 도달 가능.
- **mastery 5(mastered)는 오직 `low` scaffold 시도에서만 도달**한다 — low가 최고 등급의 유일한 경로. low를 거치지 않는 학생은 mastery 4에 머문다.
- 천장에 막혀 +1이 적용되지 않아도 감점(-1)은 정상 적용된다 (점수가 낮으면 어느 scaffold든 내려감).

- mastery는 떨어질 수도 있다 (정직한 신호).
- **`master_challenge` 시도는 mastery 갱신에서 상향만 적용, 하향은 제외**한다 (§6 참조 — 잘하면 +1, 못해도 -1 없음).
- 같은 세션·같은 단어에서 `try_again`을 여러 번 하면, mastery 반영은 **그 단어의 마지막 시도 점수 1개만** 사용한다 (try_again은 학습 기회이므로 중간 실패에 페널티 없음).

### 4-4. SRS — mastery 기반 노출 우선순위 모델

> **JaneDuck의 SRS는 전통 SM-2가 아니다.** SM-2식 상태 머신(state enum, ease_factor, interval 공식, lapse_count 등)을 쓰지 않는다.
> 대신 단어마다 **"노출 우선순위(review_priority)" 점수**를 매번 계산해, 큐가 그 점수로 복습 단어를 뽑는다.

#### 왜 SM-2를 쓰지 않는가
- JaneDuck은 AI가 0~10 작문 점수를 준다 — SM-2가 자가평가만으로 난이도를 추정하던 것보다 훨씬 풍부한 신호. ease_factor 같은 보정 장치가 불필요하다.
- "이 단어가 이 학생에게 어렵다"는 정보는 **mastery가 안 오르는 것**으로 이미 드러난다. ease_factor가 하던 일을 mastery + 자가평가가 대신한다.
- 세 신호 모델(§4)과 일관 — SM-2의 state/ease_factor는 네 번째·다섯 번째 개념을 끌어들여 설계를 흐린다.

#### 단어가 SRS를 위해 들고 있는 것 (그게 전부)
- `mastery_level` (0~5) — §4-3으로 갱신
- `last_self_eval` (`dont_know`/`unsure`/`know`/`know_well`, 또는 없음)
- `last_reviewed_at` — 이 단어를 마지막으로 학습한 시각
- `writing_count` — 작문 시도 누적 횟수

> SRS state enum, `ease_factor`, `interval_days`, `lapse_count`, `previous_interval_days` 등 **SM-2 필드는 두지 않는다.**

#### 노출 우선순위(review_priority)
각 단어에 대해 "지금 이 단어가 얼마나 복습이 급한가"를 매번 계산한다. **달력상 며칠 뒤"라는 마감일 개념이 없다.** 학생이 며칠 만에 와도 그 시점에 점수를 다시 계산할 뿐 — 폭탄도 늪도 없다.

review_priority가 높아지는 요인:
- `mastery_level`이 낮을수록 ↑ (덜 익은 단어)
- `last_reviewed_at`이 오래됐을수록 ↑ (오래 안 본 단어)
- `last_self_eval`이 "어렵다"(`dont_know`/`unsure`)일수록 ↑ — **mastery가 높아도 자가평가가 어렵다면 더 노출**
- `writing_count`가 적을수록 ↑ (덜 써본 단어)

- 자가평가는 **선택사항**(reference용, 안 해도 됨). 했으면 위 계산에 반영, 안 했으면 그 요인은 0.
- 구체적인 가중치 계산식·큐 비율 수치는 **`SRS_SPEC.md`**에 정의돼 있다. 구현은 그 명세를 따른다.

#### 큐와의 관계
- review_priority는 단어 *개별*의 시급도다. 한 세션에 어떤 단어 20장을 뽑을지는 **카드 큐(§3)**가 결정한다 — 둘은 다른 시스템.
- 큐는 review_priority 높은 순으로 복습 단어를 뽑되, new 비율을 보장한다 (§3 — "복습만 나오지 않는다").
...
> 이 경계는 §4-3의 scaffold 천장과 맞물려 있다. high 천장은 mastery 2이므로, mastery 2에 도달하면 다음 만남부터 `medium`을 줘야 학생이 3 이상으로 올라갈 길이 열린다 (high만 계속 주면 천장 2에 막혀 데드락). 같은 이유로 medium 천장은 4, mastery 5는 학생이 `master_challenge`로 low에 도전하거나 자연히 low를 만났을 때 도달한다.

scaffold는 **언제나 mastery 기반**이다. self_eval은 scaffold 결정에 쓰지 않는다 (주관적 인지 ≠ 산출 능력). self_eval의 용도는 카드 큐 정렬·Phase 2 분석·UI 표시.
처음 만나는 단어(mastery 0)도 high에서 시작 — 작문 능력은 self_eval로 측정 불가하므로. 잘하는 학생은 첫 시도 8+점이면 mastery가 올라 다음 만남부터 medium 쪽으로 이동한다.
...
- `writing_rating`의 `again` 등급은 **분석·기록용 라벨일 뿐**, UX상의 강제 행동이 아니다. 낮은 점수는 mastery를 낮춰(§4-3) 그 단어의 노출 우선순위를 높일 뿐(§4-4), 학생을 강제로 재작문시키지 않는다.
...
- master_challenge 시도의 점수는 `writing_attempts`에 저장된다. mastery 갱신은 **상향만 적용, 하향은 제외**한다:
  - `ai_score ≥ 8` → mastery **+1 적용** (해당 시도 scaffold의 천장 룰을 따름 — §4-3). master_challenge는 scaffold를 올려 시도하므로, medium 학생이 low로 도전해 8점을 받으면 이 경로로 mastery가 오른다. **mastery 4 → 5는 이 경로(low scaffold)로만 도달한다.**
  - `ai_score ≤ 4` → mastery **-1 적용 안 함** (하향 보호 — 도전이 손해가 되지 않도록).
- 즉 master_challenge는 "올라갈 수는 있고 내려가지는 않는" 시도다. 잘하면 보상, 못해도 페널티 없음.
...
`writing_rating`은 `ai_score`를 4단계 라벨로 요약한 값이다. **SRS 노출은 mastery(§4-3, §4-4)가 결정하므로 writing_rating이 노출 간격을 직접 굴리지 않는다.** writing_rating은 Phase 2 분석·기록·UI 표시용 라벨이다.
...
(자가평가 경로에서는 학생이 같은 4단계를 직접 선택할 수 있으나, 선택은 선택사항이다 — §4-4.)
...
- `user_id`, `card_id`
- `last_self_eval_rating`: enum (`dont_know`/`unsure`/`know`/`know_well`) nullable, `last_self_eval_at`: timestamp nullable — 자가평가는 선택사항 (§4-4)
- `mastery_level`: int 0~5
- `recent_scores`: array (최근 3개 — mastery 갱신/scaffold 결정용)
- `writing_attempts_count`: int — = `writing_count` (작문 시도 누적)
- `last_writing_score`: int, `last_writing_at`: timestamp
- `current_scaffold`: enum (`high`/`medium`/`low`)
- `last_reviewed_at`: timestamp — 이 단어를 마지막으로 학습한 시각 (노출 우선순위 계산용 — §4-4)
- `is_starred`: boolean default false, `starred_at`: timestamp nullable

> **SM-2 필드를 두지 않는다** (§4-4). SRS state enum(`new/learning/review/relearning`), `ease_factor`, `interval_days`, `lapse_count`, `previous_interval_days` 등은 만들지 않는다.
> SRS 노출은 위 필드들(`mastery_level`, `last_self_eval_rating`, `last_reviewed_at`, `writing_attempts_count`)로 계산하는 `review_priority` 점수가 결정한다. 계산식은 `SRS_SPEC.md` 참조.
